...
namespace ZeusInspector.SourceGenerators
{
  ...
  public class ScriptPropertiesGenerator : ISourceGenerator
  {
    ...
    private static IEnumerable<PropertyInfo> DetermineGroupingPropertyInfo(ISymbol memberSymbol)
    {
      ...
      {
        string attrName = attr.AttributeClass?.FullQualifiedNameOmitGlobal();
        PropertyUsageFlags? propertyUsage = attrName switch
        {
        ...
        if (attr.ConstructorArguments.Length > 0 && attr.ConstructorArguments[0].Value is string name)
        {
          ...
          if (attrName == GodotClasses.ExportSubgroupAttr || attrName == GodotClasses.ExportGroupAttr)
          {
            if (attr.ConstructorArguments.Length > 1)
              hintString = attr.ConstructorArguments[1].Value?.ToString();
          }

          else if (attrName == ZeusClasses.ExportGroupAttr)
          {
            hintString = attr.ConstructorArguments[1].Value?.ToString();
            hintString += ",";
            hintString += "0";
            hintString += ",";
            hintString += (int)attr.ConstructorArguments[2].Value!;
    ...
    )
    {
      ...
      if (!isTypeArgument && variantType == VariantType.Dictionary)
      {
        ...
        {
          if (type.TypeKind == TypeKind.Struct && (type as INamedTypeSymbol).IsSerializable)
          {
            hint = PropertyHint.None;
            hintString = type.Name;
            return true;
          }
